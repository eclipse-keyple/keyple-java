package org.keyple.calypso.commandsSet.transaction;
...
public class PoPlainSecureSessionTest {
    ...
    public void processOpeningTest_no_insideCommand() throws ReaderException {
        ...
        OpenSessionCmdBuild openCommand = new OpenSessionCmdBuild(PoRevision.CLASS_0x94, key, samchallenge, sfi, record);
        PoPlainSecureSession poPlainSecureSession = new PoPlainSecureSession(poReader, csmSessionReader);
        SendableInSession[] poCommandsInsideSession = new SendableInSession[1];

        Mockito.when(poReader.transmit(any(SERequest.class))).thenReturn(seResponsePO);
        Mockito.when(csmSessionReader.transmit(any(SERequest.class))).thenReturn(seResponseCSM1);
        poCommandsInsideSession[0] = new ReadRecordsCmdBuild(PoRevision.CLASS_0x94, record, enumSFI.EVENT_LOG_FILE.getSfi(),
                (byte) 0x00);
    ...
    public void processProceedingTest() throws ReaderException {
        ...
        poCommandsInsideSession[1] = new ReadRecordsCmdBuild(PoRevision.CLASS_0x94, record, enumSFI.EVENT_LOG_FILE.getSfi(),
                (byte) 0x00);
        poCommandsInsideSession[0] = new UpdateRecordCmdBuild(PoRevision.CLASS_0x94, record,
    ...
    public void processClosingTest() throws ReaderException {
        ...
        CloseSessionCmdBuild closeCommand = new CloseSessionCmdBuild(PoRevision.CLASS_0x94, true, samChallenge);
        PoGetChallengeCmdBuild poGetChallengeCmdBuild = new PoGetChallengeCmdBuild(PoRevision.CLASS_0x94);
