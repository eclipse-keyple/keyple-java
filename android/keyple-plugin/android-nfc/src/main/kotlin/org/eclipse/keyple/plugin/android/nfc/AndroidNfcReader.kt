...
/**
 * Implementation of [InterfaceAndroidNfcReader] based on keyple core abstract classes [AbstractObservableLocalReader]
 * and
 */
abstract class AndroidNfcReader : AbstractObservableLocalReader(InterfaceAndroidNfcReader.PLUGIN_NAME, InterfaceAndroidNfcReader.READER_NAME),
        InterfaceAndroidNfcReader, NfcAdapter.ReaderCallback, WaitForCardInsertionAutonomous {
    ...
    val flags: Int
        get() {
            ...
            val ndef = parameters[InterfaceAndroidNfcReader.FLAG_READER_SKIP_NDEF_CHECK]
            val nosounds = parameters[InterfaceAndroidNfcReader.FLAG_READER_NO_PLATFORM_SOUNDS]
    ...
    val options: Bundle
        get() {
            ...
            if (parameters.containsKey(InterfaceAndroidNfcReader.FLAG_READER_PRESENCE_CHECK_DELAY)) {
                val delay = Integer
                        .parseInt(parameters[InterfaceAndroidNfcReader.FLAG_READER_PRESENCE_CHECK_DELAY]!!)
...
//    }
    ...
    override fun setParameter(key: String, value: String) {
        ...
        Timber.i("%s, FLAG_READER_SKIP_NDEF_CHECK:%s, FLAG_READER_NO_PLATFORM_SOUNDS:%s, FLAG_READER_PRESENCE_CHECK_DELAY:%s",
                InterfaceAndroidNfcReader.READER_NAME,
                parameters[InterfaceAndroidNfcReader.FLAG_READER_SKIP_NDEF_CHECK],
                parameters[InterfaceAndroidNfcReader.FLAG_READER_NO_PLATFORM_SOUNDS],
                parameters[InterfaceAndroidNfcReader.FLAG_READER_PRESENCE_CHECK_DELAY])

        val correctParameter = (key == InterfaceAndroidNfcReader.FLAG_READER_SKIP_NDEF_CHECK && value == "1" || value == "0" ||
                key == InterfaceAndroidNfcReader.FLAG_READER_NO_PLATFORM_SOUNDS && value == "1" || value == "0" ||
                key == InterfaceAndroidNfcReader.FLAG_READER_PRESENCE_CHECK_DELAY && Integer.parseInt(value) > -1)
