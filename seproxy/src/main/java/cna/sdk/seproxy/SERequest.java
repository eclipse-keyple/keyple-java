...
public class SERequest {
    ...
    public SERequest(byte[] aidToSelect, boolean keepChannelOpen, List<APDURequest> apduRequests) {
        ...
        if (aidToSelect != null && aidToSelect.length > 0) {
            this.apduRequests = new ArrayList<>();

            byte[] request = new byte[6 + aidToSelect.length];

            request[0] = (byte) 0x00;
            request[1] = (byte) 0xA4;
            request[2] = (byte) 0x04;
            request[3] = (byte) 0x00;
            request[4] = (byte) aidToSelect.length;
            System.arraycopy(aidToSelect, 0, request, 5, aidToSelect.length);
            request[5 + aidToSelect.length] = (byte) 0x00;
            APDURequest aidRequest = new APDURequest(request, false);
            this.apduRequests.add(aidRequest);
