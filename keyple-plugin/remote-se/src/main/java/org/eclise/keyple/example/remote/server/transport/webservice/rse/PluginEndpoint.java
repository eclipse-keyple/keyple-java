...
public class PluginEndpoint implements HttpHandler {
    ...
    private void onReaderConnect(HttpExchange t) throws IOException {
        ...
        Boolean isDuplex = body.get("isDuplex").getAsBoolean();
        ...
        if(!isDuplex){
            ...
            plugin.connectRemoteReader(readerName, new org.eclise.keyple.example.remote.server.transport.webservice.rse.VirtualNseSession(sessionId));
        }else{
            plugin.connectRemoteReader(readerName, new WsNseClient(serverUrl, sessionId));
        }
    ...
    private void onReaderEvent(HttpExchange t) throws IOException {
        ...
        try {
            ...
            RseNseSession session = reader.getSession();

            //if session has not double way communication
            if (!session.isDuplex()){
               // there are SeRequestSet to process, attach them to the response
              if(((SneSession)session).hasSeRequestSet()) {
                  responseBody = SeProxyJsonParser.getGson().toJson(((SneSession)session).getSeRequestSet());
              } else {
                  responseBody = "{}";
              }
            }else{
