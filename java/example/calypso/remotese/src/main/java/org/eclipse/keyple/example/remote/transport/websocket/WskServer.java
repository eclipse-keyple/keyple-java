...
class WskServer extends WebSocketServer implements ServerNode {
                ...
                if (dtoHandler != null) {
                  ...
                  if (isMaster) {
                    ...
                    if (keypleDto.getNativeReaderName() != null) {
                      ...
                      addConnection(
                          conn, keypleDto.getNativeReaderName(), keypleDto.getRequesterNodeId());
                    } else {
                      logger.debug(
  ...
  private final Map<String, WebSocket> nativeReaderName_session = new HashMap<String, WebSocket>();
  ...
  private WebSocket addConnection(
      WebSocket connection, String nativeReaderName, String clientNodeId) {
  ...
  public void sendDTO(TransportDto transportDto) {
    logger.trace(
        "sendDTO with a TransportDto {} {}", KeypleDtoHelper.toJson(transportDto.getKeypleDTO()));
          ...
          if (transportDto.getKeypleDTO().getSessionId() == null) {
            ...
            if (conn == null) {
              throw new KeypleRuntimeException(
                  "Conn was not found for "
                      + transportDto.getKeypleDTO().getNativeReaderName()
                      + " "
                      + transportDto.getKeypleDTO().getTargetNodeId());
            }
            logger.trace(
                "send DTO with websocket {} {}",
  ...
  public void sendDTO(KeypleDto message) {
    logger.trace(
        "Web socket sendDTO without predefined socket {}", KeypleDtoHelper.toJson(message));
