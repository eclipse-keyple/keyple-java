...
/**
 * Rest client, polls server, based on java.net client and multi threads @Deprecated
 *
 * <p>Use {@link WsPollingFactory} instead
 */
@Deprecated
public class WsPClient implements ClientNode {
  ...
  private final String keypleDtoEndpoint;
            ...
            while (true) {
              try {
                ...
                JsonObject httpResponse =
                    httpPoll(
                        getConnection(baseUrl + pollingEndpoint + "?clientNodeId=" + clientNodeId),
                        "{}");
                logger.debug(
                    "Polling for clientNodeId {} receive a httpResponse {}",
                    clientNodeId,
                    httpResponse);
  ...
  public void sendDTO(TransportDto transportDto) {
    ...
    if (!KeypleDtoHelper.isNoResponse(transportDto.getKeypleDTO())) {
      try {
        ...
        JsonObject httpResponse =
            httpPOSTJson(
                getConnection(baseUrl + keypleDtoEndpoint), KeypleDtoHelper.toJson(keypleDto));
