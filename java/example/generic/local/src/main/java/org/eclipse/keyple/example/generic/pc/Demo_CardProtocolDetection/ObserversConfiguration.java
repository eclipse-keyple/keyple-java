    ...
    public final void update(final ReaderEvent event) {
      ...
      switch (event.getEventType()) {
        case CARD_INSERTED:
          ...
          break;
        case CARD_MATCHED:
          CardSelection cardSelection = SelectionConfiguration.prepareCardSelection();
          /* get the card that matches one of the two selection targets */
          if (cardSelection.processDefaultSelection(event.getDefaultSelectionsResponse()).hasActiveSelection()) {
            AbstractSmartCard selectedCard =
                    cardSelection.processDefaultSelection(event.getDefaultSelectionsResponse()).getActiveSmartCard();
          }
          break;
        case CARD_REMOVED:
