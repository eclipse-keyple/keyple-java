# keyple-example-remote-pool-web-service project

This projects aims at demonstrating how straight forward is the implementation of the synchronous mode of the Pool Remote Plugin. 
...
- client executes a complete transaction
- client requests to release the reader
- server shutdowns

By default, this example uses the keyple `StubPoolPlugin` that emulates a Native Reader and a Card. You can activate the PCSC configuration by invoking ```initPcscReader()``` instead of  ```initStubReader()``` in the `clientApp.init()` method.      
