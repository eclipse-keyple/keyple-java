 ...
 ************************************************************************************** */
package org.eclipse.keyple.example.generic.remote.server.webservice.client;
...
import org.eclipse.keyple.example.generic.remote.server.webservice.util.CalypsoSmartCard;
import org.eclipse.keyple.example.generic.remote.server.webservice.util.UserInputDataDto;
import org.eclipse.keyple.example.generic.remote.server.webservice.util.UserOutputDataDto;
...
public class AppClient {
   ...
   *       org.eclipse.keyple.plugin.remote.spi.SyncEndpointClient} endpoint.
   * </ul>
   */
  public void init() {
  ...
  private void initStubReader() {
    ...
    plugin = SmartCardService.getInstance().registerPlugin(new StubPluginFactory("stubPlugin"));

    // Plug the reader manually to the plugin.
    ((StubPlugin) plugin).plugStubReader("stubReader", true);
  ...
  private void initPcscReader() {
    ...
    // Registers the plugin to the smart card service.
    plugin = SmartCardService.getInstance().registerPlugin(new PcscPluginFactory());
