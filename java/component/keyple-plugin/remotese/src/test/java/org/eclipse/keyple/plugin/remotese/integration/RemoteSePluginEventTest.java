...
public class RemoteSePluginEventTest extends VirtualReaderBaseTest {
   ...
   *
   * @throws Exception @Test(expected = KeypleReaderException.class) public void
   *     testConnectTwiceSameReader() throws Exception { // lock test until message is received
   *     final CountDownLatch lock = new CountDownLatch(1);
   *     <p>obs = new ObservablePlugin.PluginObserver() { @Override public void update(PluginEvent
   *     event) { // READER_CONNECTED should be raised only once, so lock.getCount() should be
   *     equals // to 1 if (1 != lock.getCount()) { throw new IllegalStateException();
   *     //Assert.assertTrue(false); } Assert.assertNotNull(event.getReaderNames().first());
   *     Assert.assertEquals(1, event.getReaderNames().size());
   *     Assert.assertEquals(remoteSePlugin.getName(), event.getPluginName());
   *     Assert.assertEquals(PluginEvent.EventType.READER_CONNECTED, event.getEventType());
   *     lock.countDown(); } }; //register observer remoteSePlugin.addObserver(obs);
   *     <p>// connect a Stub Native reader try { this.connectStubReader(NATIVE_READER_NAME,
   *     CLIENT_NODE_ID, TransmissionMode.CONTACTLESS); } catch (Exception e) { e.printStackTrace();
   *     }
   *     <p>// wait 2 seconds lock.await(2, TimeUnit.SECONDS);
   *     <p>// connect twice try { this.connectStubReader(NATIVE_READER_NAME, CLIENT_NODE_ID,
   *     TransmissionMode.CONTACTLESS); //should throw exception } catch (Exception e) {
   *     //Disconnect reader to clean environment disconnectReader(NATIVE_READER_NAME); //propagate
   *     exception throw e; } // Expect a KeypleReaderException exception to be thrown }
   */
  ...
  public void testMaster_DisconnectReaderNotFound() throws Exception {
    ...
    masterAPI
        .getPlugin()
        .disconnectVirtualReader(((VirtualReader) virtualReader).getNativeReaderName(), "none");
  }
  ...
  public void testMaster_DisconnectReaderNotFound2() throws Exception {
    ...
    masterAPI
        .getPlugin()
        .disconnectVirtualReader("none", virtualReader.getSession().getSlaveNodeId());
  }
}
