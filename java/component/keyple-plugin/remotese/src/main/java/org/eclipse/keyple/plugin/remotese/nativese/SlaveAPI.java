...
public class SlaveAPI implements INativeReaderService, DtoHandler, ObservableReader.ReaderObserver {
   ...
   * @return a transportDto (can be a NoResponse KeypleDto). If an exception is thrown during onDTO
   *     processing, a keyple dto exception is returned
   */
  @Override
  public TransportDto onDTO(TransportDto transportDto) {
    ...
    logger.trace(
        "{} Remote Method called : {} - isRequest : {}",
        dtoNode.getNodeId(),
        method,
        keypleDTO.isRequest());
    ...
    try {
      ...
      switch (method) {
        ...
        case READER_DISCONNECT:
          if (keypleDTO.isRequest()) {
            throw new IllegalStateException(
                "a " + keypleDTO.getAction() + " request has been received by SlaveAPI");
          } else {
        ...
        default:
          ...
          logger.warn(
              "Receive unrecognized message action : {} {} {} {}",
              keypleDTO.getAction(),
  ...
  public String connectReader(Reader localReader, Map<String, String> options) {
    ...
    logger.trace(
        "{} connectReader {} from device {}",
        dtoNode.getNodeId(),
        localReader.getName(),
        dtoNode.getNodeId());

    RmConnectReaderTx connect =
        new RmConnectReaderTx(
            null,
            localReader.getName(),
            null,
            masterNodeId,
  ...
  public void disconnectReader(String sessionId, String nativeReaderName) {
    ...
    try {
      ...
      throw new KeypleReaderIOException("An error occurred while calling disconnectReader", e);
  ...
  /**
   * Do not call this method directly This method is called by a Observable&lt;{@link
   * ReaderEvent}&gt;
   *
    ...
    try {
      dtoNode.sendDTO(
          KeypleDtoHelper.buildNotification(
              RemoteMethodName.READER_EVENT.getName(),
              data,
              null,
              event.getReaderName(),
              null,
              this.dtoNode.getNodeId(),
              masterNodeId));
    } catch (KeypleRemoteException e) {
      logger.error(
          "Event " + event.toString() + " could not be sent though Remote Service Interface", e);
    }
  }
