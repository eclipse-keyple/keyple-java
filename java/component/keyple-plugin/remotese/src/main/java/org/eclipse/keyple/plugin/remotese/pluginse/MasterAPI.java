 ...
 *
 * <p>Init this API with a {@link DtoSender} of your implementation. Link this API to one your
 * {@link DtoHandler}.
 */
public class MasterAPI implements DtoHandler {
   ...
   * @param rpcTimeout : timeout in milliseconds to wait for an answer from slave before throwing an
   *     exception
   * @param pluginType : either a default plugin or readerPool plugin, use {@link
   *     #PLUGIN_TYPE_DEFAULT} or @PLUGIN_TYPE_POOL
  ...
  public TransportDto onDTO(TransportDto transportDto) {
    ...
    logger.trace(
        "onDTO, Remote Method called : {} - isRequest : {} - keypleDto : {}",
        method,
        keypleDTO.isRequest(),
    ...
    try {
      switch (method) {
        ...
        case DEFAULT_SELECTION_REQUEST:
          if (keypleDTO.isRequest()) {
            throw new IllegalStateException(
                "a " + keypleDTO.getAction() + " request has been received by MasterAPI");
          }
        ...
        case POOL_RELEASE_READER:
           ...
           */
          return ((RemoteSePoolPluginImpl) plugin).getRmTxEngine().onResponseDto(transportDto);
