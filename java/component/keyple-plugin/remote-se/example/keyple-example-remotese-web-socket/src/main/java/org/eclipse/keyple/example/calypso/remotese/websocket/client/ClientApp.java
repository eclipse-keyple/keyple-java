...
public class ClientApp {
    ...
    private NativeSeClientService nativeService;
    private ReaderPlugin nativePlugin;
    private SeReader nativeReader;

    /**
     * Initialize the client components : {@link StubPlugin} with a {@link StubReader} that accepts {@link org.eclipse.keyple.plugin.stub.StubSecureElement} based on protocol SeCommonProtocols.PROTOCOL_ISO14443_4.
     * Initialize the nativeSeService with and async endpoint {@link WebsocketClientEndpoint}
     */
    public void init() {
    ...
    public Boolean launchScenario() {
        ...
        TransactionResult output =
                nativeService.executeRemoteService(
                        RemoteServiceParameters.builder("EXECUTE_CALYPSO_SESSION_FROM_REMOTE_SELECTION", nativeReader)
    ...
    private void initStubReader() {
        ...
        // register plugin
        nativePlugin = SeProxyService.getInstance()
                .registerPlugin(new StubPluginFactory(STUB_PLUGIN_NAME));

        // configure native reader
        ((StubPlugin)
                nativePlugin).plugStubReader(STUB_READER_NAME, true);
        ...
        LOGGER.info("Client - Native reader was configured with STUB reader : {} with a SE", nativeReader.getName());

    }

    /**
     * Init Native Reader with a connected PCSC plugin whose name is PCSC_READER_NAME and an inserted SE
     */
    private void initPcscReader() {

        // register plugin
        nativePlugin = SeProxyService.getInstance()
                .registerPlugin(new PcscPluginFactory());

        if (nativePlugin.getReaders().size() != 1) {
            throw new IllegalStateException("For the matter of this example, we expect one and only one pcsc reader to be connected");
        }
        ...
        if (!nativeReader.isSePresent()) {
            throw new IllegalStateException("For the matter of this example, we expect a SE to be present at the startup");
        }
        ...
        LOGGER.info("Client - Native reader was configured with PCSC reader : {} with a SE", nativeReader.getName());
