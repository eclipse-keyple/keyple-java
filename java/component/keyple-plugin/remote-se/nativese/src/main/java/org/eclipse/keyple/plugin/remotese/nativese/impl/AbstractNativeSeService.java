...
abstract class AbstractNativeSeService extends AbstractKeypleMessageHandler {
    ...
    protected ProxyReader findLocalReader(String nativeReaderName) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("Try to find local reader by name '{}' in {} plugin(s)", nativeReaderName,
        ...
        for (ReaderPlugin plugin : SeProxyService.getInstance().getPlugins().values()) {
            try {
                if (logger.isTraceEnabled()) {
                    logger.trace("Try to find local reader '{}' in plugin {}", nativeReaderName,
                            plugin.getName());
                }
    ...
    private static final class NativeReaderExecutor {
        ...
        private KeypleMessageDto transmit() {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("Execute locally seRequest : {} with params {} on reader {}",
                        seRequest, channelControl, reader.getName());
            }
        ...
        private KeypleMessageDto transmitSet() {
            ...
            MultiSeRequestProcessing multiSeRequestProcessing = MultiSeRequestProcessing
        ...
        private KeypleMessageDto setDefaultSelection() {
            ...
            DefaultSelectionsRequest defaultSelectionsRequest = KeypleJsonParser.getParser()
                    .fromJson(jsonObject.get("defaultSelectionsRequest"),
        ...
        private KeypleMessageDto addSeProtocolSetting() {
            ...
            SeProtocol seProtocol = KeypleJsonParser.getParser()
        ...
        private KeypleMessageDto setSeProtocolSetting() {
            ...
            Map<SeProtocol, String> protocolSetting = KeypleJsonParser.getParser().fromJson(
        ...
        private KeypleMessageDto getTransmissionMode() {
            ...
            // Build response
            String body =
                    KeypleJsonParser.getParser().toJson(transmissionMode, TransmissionMode.class);
            return new KeypleMessageDto(msg).setBody(body);
        }
    }
