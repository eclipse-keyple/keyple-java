...
public class CardCipherPinCmdBuildTest {
    ...
    private static final KeyReference KEY_REFERENCE_CIPH_KEY = new KeyReference(ByteArrayUtil.fromHex(KIF_CIPH_KEY)[0], ByteArrayUtil.fromHex(KVC_CIPH_KEY)[0]);
    private static final String CURRENT_PIN = "31323334";
    private static final String NEW_PIN = "35363738";
    private static final String APDU_ISO_CHANGE_PIN = "80 12 40FF 0A" + KIF_CIPH_KEY +KVC_CIPH_KEY + CURRENT_PIN +NEW_PIN;
    private static final String APDU_ISO_VERIFY_PIN = "80 12 80FF 06" + KIF_CIPH_KEY +KVC_CIPH_KEY + CURRENT_PIN;

    @Test
    public void cardCipherPinCmdBuild_update_PIN() {
        CardCipherPinCmdBuild builder = new CardCipherPinCmdBuild(SamRevision.C1, KEY_REFERENCE_CIPH_KEY, ByteArrayUtil.fromHex(CURRENT_PIN), ByteArrayUtil.fromHex(NEW_PIN));
    ...
    public void cardCipherPinCmdBuild_verify_PIN() {
        CardCipherPinCmdBuild builder = new CardCipherPinCmdBuild(SamRevision.C1, KEY_REFERENCE_CIPH_KEY, ByteArrayUtil.fromHex(CURRENT_PIN), null);
        byte[] apduRequestBytes = builder.getApduRequest().getBytes();
    ...
    public void cardCipherPinCmdBuild_update_PIN_bad_length_1() {
        CardCipherPinCmdBuild builder = new CardCipherPinCmdBuild(SamRevision.C1, KEY_REFERENCE_CIPH_KEY, ByteArrayUtil.fromHex(CURRENT_PIN + CURRENT_PIN), ByteArrayUtil.fromHex(NEW_PIN));
        shouldHaveThrown(IllegalArgumentException.class);
    }

    @Test(expected = IllegalArgumentException.class)
    public void cardCipherPinCmdBuild_update_PIN_bad_length_2() {
        CardCipherPinCmdBuild builder = new CardCipherPinCmdBuild(SamRevision.C1, KEY_REFERENCE_CIPH_KEY, ByteArrayUtil.fromHex(CURRENT_PIN), ByteArrayUtil.fromHex(NEW_PIN  + NEW_PIN));
        shouldHaveThrown(IllegalArgumentException.class);
    }
