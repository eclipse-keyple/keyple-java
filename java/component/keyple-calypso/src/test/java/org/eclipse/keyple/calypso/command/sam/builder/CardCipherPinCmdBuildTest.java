...
public class CardCipherPinCmdBuildTest {
    ...
    private static final byte[] APDU_ISO_CHANGE_PIN = ByteArrayUtil.fromHex(
            "80 12 40FF 0A" + KIF_CIPH_KEY_STR + KVC_CIPH_KEY_STR + CURRENT_PIN_STR + NEW_PIN_STR);
    private static final byte[] APDU_ISO_VERIFY_PIN = ByteArrayUtil
            .fromHex("80 12 80FF 06" + KIF_CIPH_KEY_STR + KVC_CIPH_KEY_STR + CURRENT_PIN_STR);

    @Test
    public void cardCipherPinCmdBuild_update_PIN() {
        CardCipherPinCmdBuild builder = new CardCipherPinCmdBuild(SamRevision.C1,
                KEY_REFERENCE_CIPH_KEY, CURRENT_PIN, NEW_PIN);
    ...
    public void cardCipherPinCmdBuild_verify_PIN() {
        CardCipherPinCmdBuild builder = new CardCipherPinCmdBuild(SamRevision.C1,
                KEY_REFERENCE_CIPH_KEY, CURRENT_PIN, null);
        byte[] apduRequestBytes = builder.getApduRequest().getBytes();
    ...
    public void cardCipherPinCmdBuild_update_PIN_bad_length_1() {
        CardCipherPinCmdBuild builder = new CardCipherPinCmdBuild(SamRevision.C1,
                KEY_REFERENCE_CIPH_KEY, PIN_LENGTH_5, NEW_PIN);
    }

    @Test(expected = IllegalArgumentException.class)
    public void cardCipherPinCmdBuild_update_PIN_bad_length_2() {
        CardCipherPinCmdBuild builder = new CardCipherPinCmdBuild(SamRevision.C1,
                KEY_REFERENCE_CIPH_KEY, CURRENT_PIN, PIN_LENGTH_5);
    }
