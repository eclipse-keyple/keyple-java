 ...
 ************************************************************************************** */
package org.eclipse.keyple.core.reader.message;
...
public final class ApduResponse implements Serializable {
   ...
   *
   * <p>The internal successful status is determined by the current status code and the optional
   * successful status codes list.
   *
   * <p>The list of additional successful status codes is used to set the successful flag if not
   * equal to 0x9000
   *
  ...
  public ApduResponse(byte[] buffer, Set<Integer> successfulStatusCodes) {
    ...
    if (buffer == null) {
      ...
      int statusCode =
          ((buffer[buffer.length - 2] & 0x000000FF) << 8)
  ...
  public int getStatusCode() {
    int s = ((bytes[bytes.length - 2] & 0x000000FF) << 8) + (bytes[bytes.length - 1] & 0x000000FF);
